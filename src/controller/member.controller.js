...
async function createMember(req, res) {
   ...
   //if user is admin of the organization, return 400
       const isAdmin = await organizationModel.findOne({ _id: organizationId, adminId: userId });
...
async function deleteMember(req, res) {
    ...
    if (!userId || !memberId || !organizationId) {
        return res.status(400).json({ message: "User ID, Member ID and Organization ID are required" });
    }

    if ( !mongoose.Types.ObjectId.isValid(organizationId) || !mongoose.Types.ObjectId.isValid(memberId)) {
        return res.status(400).json({ message: "Invalid User ID, Member ID or Organization ID" });
    }
    ...
    try {
      ...
      if(isAdmin || isSameUser){
        ...
        return res.status(403).json({ message: "You are not authorized to delete this member" });
      }
    }catch(err){
